...
def get_word_score(word, HAND_SIZE):
    ...
    score2 = max(7*len(word)-3*(HAND_SIZE-len(word)), 1)
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list:
        ...
        index = word.find('*')
        for vowel in VOWELS:
            if word[0:index] + vowel + word[index+1:] in word_list:
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(word, hand, word_list):
                ...
                totscore += get_word_score(word, calculate_handlen(hand))
                print('Word total:', totscore)
    ...
    if calculate_handlen(update_hand(hand, word)) == 0:
        print('Ran out of letters.')
    # so tell user the total score
    print('Total score for this hand:', totscore)
    print('- - - - - - - - - -')
...
def substitute_hand(hand, letter):
    ...
    letter_amount = hand[letter]
    new_hand = hand.copy()
    del(new_hand[letter])
    new_letter = random.choice(all_letters)
    new_hand[new_letter] = letter_amount
    return new_hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    word_list: list of lowercase strings
    """
    tothands = int(input('Enter total number of hands: '))
    totscore = 0
    subcount = 1
    replays = 1
    while tothands > 0:
        ...
        while subcount > 0:
            display_hand(hand)
            sub = input('Would you like to substitute a letter? Enter "yes" or "no": ')
            sub = sub.lower()
            if sub != 'yes' and sub != 'no':
                print('You must type "yes" or "no".')
                continue
            if sub == 'yes':
                old_letter = input('Enter the letter you would like to replace: ')
                hand = substitute_hand(hand, old_letter)
                subcount -= 1
            else:
                break
        score = play_hand(hand, word_list)
        while replays > 0:
            repl = input('Would you like to replay the hand? Enter "yes" or "no": ')
            if repl == 'yes':
                replays -= 1
